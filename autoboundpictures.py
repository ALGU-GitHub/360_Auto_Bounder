...
def debug_bound_info(output_path, image_name, angle_increment):
    ...
    for current_angle in np.arange(0, 360, angle_increment):
        new_image_path = image_directory + '_a{:d}.jpg'.format(current_angle)
        new_bound_info_path = image_directory + '_a{:d}.txt'.format(current_angle)
        if os.stat(new_bound_info_path).st_size != 0:
            with open(new_bound_info_path) as bound_info_file:
                ...
                while bound_info_string:
                    ...
                    bound_info_string = bound_info_file.readline()
            new_boxes_path = boxes_directory + '/' + image_name + '_a{:d}_boxes.jpg'.format(current_angle)
...
def run_detection_on_frame(current_frame, image_name):

    output_path = os.getcwd() + '/Pictures_Output';
    ...
    for angle in np.arange(0, 360, angle_increment):
        new_bound_info_path = output_path + '/' + image_name + '_a{:d}.txt'.format( angle)
    ...
    for current_angle in np.arange(0, 360, angle_increment):
        ...
        new_image_path = output_path + '/' + image_name + '_a{:d}.jpg'.format(current_angle)
        ...
        if os.stat(new_bound_info_path).st_size != 0:
            with open(bound_info_path) as bound_info_file:
                ...
                while bound_info_string:         
                    ...
                    current_bound_info_path = autobound_path + '/Output/' + image_name + '/' + image_name + '_a{:d}.txt'.format(current_angle)
            ...
            os.remove(new_image_path)
            for current_angle in np.arange(0, 360, angle_increment):
                bound_info_path_to_delete = output_path + '/' + image_name + '_a{:d}.txt'.format(current_angle)
                os.remove(bound_info_path_to_delete)
            os.chdir(autobound_path)
            return
        #new_prediction_path = output_path + '/' + image_name + '_a{:d}_prediction.jpg'.format(current_angle)
        prediction_path = os.getcwd() + '/predictions.jpg'
        #shutil.copy(prediction_path, new_image_path + '_prediction.jpg')
        os.remove(prediction_path)
        os.chdir(autobound_path)
    debug_bound_info(output_path, image_name, angle_increment)
...
def produce_dataset_from_picture(picture_path, picture_name):
    current_frame_capture = cv2.imread(picture_path)
    current_frame_with_boarders = get_frame_with_boarders(current_frame_capture)
    run_detection_on_frame(current_frame_with_boarders, picture_name)

input_path = 'Input_Pictures'
...
for file_in_input_path in os.listdir(input_path):
    if file_in_input_path.endswith('.jpg'):
        print('Working on ' + file_in_input_path + '...')
        picture_path = input_path + '/' + file_in_input_path
        picture_name = os.path.splitext(file_in_input_path)[0]
        produce_dataset_from_picture(picture_path, picture_name)
